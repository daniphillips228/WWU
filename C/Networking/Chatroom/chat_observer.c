...
*
* Purpose: allocate a socket, connect to a server, and print all chats
*
...
int main( int argc, char **argv) {
	...
	char* msg;
	while (n > 0) {
		//check if message ends with a newline character, if not add one

		n = recv(sd, &msgLength, 2, MSG_WAITALL);
		n = recv(sd, buf, msgLength, MSG_WAITALL);
		buf[msgLength] = 0;
